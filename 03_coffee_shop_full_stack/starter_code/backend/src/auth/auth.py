...
AUTH0_DOMAIN = 'dev-31xxr8yqul4ks2qk.eu.auth0.com'
ALGORITHMS = ['RS256']
...
def get_token_auth_header() -> str:
    ...
    if len(parts) != 2 or parts[0].lower() != 'bearer':
        raise AuthError({
            'code': 'invalid_header',
            'description': 'Authorization header must start with "Bearer" and contain a token.'
        }, 401)
...
def check_permissions(required_permission, payload):
    # Check if 'permissions' is present in payload
    if 'permissions' not in payload:
        raise AuthError({'code': 'invalid_claims',
                         'description': 'Permissions not included in JWT payload.'}, 400)

    # Check if required_permission is present in payload['permissions']
...
def verify_decode_jwt(token):
    ...
    if rsa_key:
        try:
            ...
            raise AuthError(
                {'code': 'invalid_header', 'description': 'Unable to parse authentication token.'}, 400)
    raise AuthError({'code': 'invalid_header',
